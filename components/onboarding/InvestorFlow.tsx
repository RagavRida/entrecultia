...
export function InvestorFlow() {
    ...
    const isStep1Valid = () => {
        return data.name && isAgeValid && data.city && isEmailValid && data.phone && data.phone.length === 10 && data.occupation && data.source && data.idFileUrl && data.otpVerified;
    };
                                    ...
                                    {otpSent && !data.otpVerified && (
                                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex flex-col gap-2">
                                            <div className="flex gap-2">
                                                ...
                                                />
                                                <Button variant="primary" onClick={handleVerifyOtp} disabled={otpInput.length < 6 || isVerifyingOtp} className="bg-[#84a98c] hover:bg-[#5a8062] h-[48px] whitespace-nowrap text-white">
